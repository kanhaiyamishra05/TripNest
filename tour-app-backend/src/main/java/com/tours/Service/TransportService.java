...
public class TransportService {
    ...
    public Transport addTransport(Transport transport) {
        ...
        if (transport.getTransportName() != null && transport.getTransportType() != null) {
            java.util.Optional<Transport> existing = transportRepository.findByTransportNameIgnoreCaseAndTransportTypeIgnoreCase(
                transport.getTransportName(), transport.getTransportType()
            );
            if (existing.isPresent()) {
                logger.info("Transport already exists. Reusing existing Transport ID: " + existing.get().getId());
                return existing.get();
            }
        }
