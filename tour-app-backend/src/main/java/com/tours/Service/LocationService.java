...
public class LocationService {
    ...
    public Location addLocation(Location location) {
        ...
        if (location.getFromLocation() != null && location.getToLocation() != null && location.getCountry() != null) {
            Optional<Location> existing = locationRepository.findByFromLocationIgnoreCaseAndToLocationIgnoreCaseAndCountryIgnoreCase(
                location.getFromLocation(), location.getToLocation(), location.getCountry()
            );
            if (existing.isPresent()) {
                logger.info("Location already exists. Reusing existing Location ID: " + existing.get().getId());
                return existing.get();
            }
        }
