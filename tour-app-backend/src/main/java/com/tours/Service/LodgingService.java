...
public class LodgingService {
    ...
    public Lodging addLodging(Lodging lodging) {
        ...
        if (lodging.getLodgingName() != null && lodging.getAddress() != null) {
            java.util.Optional<Lodging> existing = lodgingRepository.findByLodgingNameIgnoreCaseAndAddressIgnoreCase(
                lodging.getLodgingName(), lodging.getAddress()
            );
            if (existing.isPresent()) {
                logger.info("Lodging already exists. Reusing existing Lodging ID: " + existing.get().getId());
                return existing.get();
            }
        }
