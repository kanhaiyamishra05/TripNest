...
const AddTour = () => {
  ...
  const handleLocationSelectChange = (e) => {
    ...
    if (locId) {
      const selectedLoc = existingLocations.find((loc) => String(loc.id) === String(locId));
      if (selectedLoc) {
        setUserData((prev) => ({
          ...prev,
          fromLocation: selectedLoc.fromLocation || "",
          toLocation: selectedLoc.toLocation || "",
          distance: selectedLoc.distance || "",
          estimatedTravelTime: selectedLoc.estimatedTravelTime || "",
          locationDescription: selectedLoc.locationDescription || "",
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {[
              ...
              "Glacier Hike & Train Ride",
              "Desert Safari & Camel Ride",
              "Traditional Dance & Folk Show",
              "Cooking Class & Factory Tour",
            ].map((activity) => (
      ...
      {existingLocations.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
          <label className="block text-sm font-semibold text-blue-800 mb-2">
            Select Existing Location Route (Auto-fill):
          </label>
          <select
          ...
          >
            <option value="">-- Add New Location Route (Type Manually) --</option>
            {existingLocations.map((loc) => (
              <option key={loc.id} value={loc.id}>
                {loc.fromLocation} to {loc.toLocation} ({loc.country})
              </option>
            ))}
          </select>
          <p className="text-xs text-blue-600 mt-1">
            Selecting an existing route will automatically fill the location fields below.
          </p>
        </div>
      )}
