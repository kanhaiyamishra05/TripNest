...
import { adminTransport, deleteTransport } from "../../../../Redux/API/API";
import { Train, Bus, Clock, Info, Plane, Car, Trash2 } from "lucide-react";
import { toast } from "sonner";

const TransportCard = ({ transport, onDelete }) => {
  ...
  return (
    <div className="p-4 mb-4 transition-all duration-300 ease-in-out bg-white border border-gray-100 rounded-lg shadow-md hover:shadow-xl relative group">
...
const AllTransport = () => {
  ...
  const handleDelete = async (transportId) => {
    ...
    try {
      const response = await dispatch(deleteTransport(transportId));
      if (response.type.endsWith("/fulfilled")) {
        toast.success("Transport option deleted successfully");
        setTransports(transports.filter((t) => t.id !== transportId));
      } else {
        toast.error("Failed to delete transport. It might be linked to a tour.");
      }
    } catch (err) {
      console.error(err);
      ...
      {transports.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {transports.map((transport) => (
            <TransportCard key={transport.id} transport={transport} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
