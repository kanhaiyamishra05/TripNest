...
import { adminLodging, deleteLodging } from "../../../../Redux/API/API";
import { Hotel, MapPin, Star, Trash2 } from "lucide-react";
import { toast } from "sonner";

const LodgingCard = ({ lodging, onDelete }) => {
...
const AllLodging = () => {
  ...
  const handleDelete = async (lodgingId) => {
    if (!window.confirm("Are you sure you want to delete this lodging option?")) return;
    try {
      const response = await dispatch(deleteLodging(lodgingId));
      if (response.type.endsWith("/fulfilled")) {
        toast.success("Lodging option deleted successfully");
        setLodgings(lodgings.filter((l) => l.id !== lodgingId));
      } else {
        toast.error("Failed to delete lodging. It might be linked to a tour.");
      }
    } catch (err) {
      console.error(err);
      toast.error("Error deleting lodging");
    }
  };
      ...
      {filteredAndSortedLodgings.length === 0 ? (
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAndSortedLodgings.map((lodging) => (
            <LodgingCard key={lodging.id} lodging={lodging} onDelete={handleDelete} />
          ))}
        </div>
      )}
    </div>
